...
void char_movement_right(env_t *env)
{
    ...
    if (check_collision(env, (sfVector2i) {CHARACTER_SPEED * (env->core_s.delta_time * (DEFAULT_FRAMERATE / 2)), 0}))
        return;
    if (pos.x < (env->game_s.map_size.width - (1920 / 2) * 0.6)
    && pos.x == env->game_s.p_character.x)
        sfView_move(env->game_s.view, (sfVector2f) {CHARACTER_SPEED * (env->core_s.delta_time * (DEFAULT_FRAMERATE / 2)), 0});
    if (env->game_s.p_character.x + (32 / 2) < env->game_s.map_size.width) {
        set_character_animation(env);
        env->game_s.p_character.x += CHARACTER_SPEED * (env->core_s.delta_time * (DEFAULT_FRAMERATE / 2));
    }
}
...
void char_movement_top(env_t *env)
{
    ...
    if (check_collision(env, (sfVector2i) {0, -CHARACTER_SPEED * (env->core_s.delta_time * (DEFAULT_FRAMERATE / 2))}))
        return;
    if (pos.y > (1080 / 2) * 0.6
    && pos.y == env->game_s.p_character.y)
        sfView_move(env->game_s.view, (sfVector2f) {0, -CHARACTER_SPEED * (env->core_s.delta_time * (DEFAULT_FRAMERATE / 2))});
    if (env->game_s.p_character.y - (32 / 2) > 0) {
        set_character_animation(env);
        env->game_s.p_character.y -= CHARACTER_SPEED * (env->core_s.delta_time * (DEFAULT_FRAMERATE / 2));
    }
}
